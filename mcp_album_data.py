...
async def fetch_release_by_name(artist_name: str, release_title: str) -> dict:
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(
            "SELECT album_artists, album_title, label, catalog_id, release_date FROM merged_data WHERE album_artists = ? AND album_title = ?",
...
@mcp.tool()
async def prepare_release_for_musicbrainz(release_id: int) -> dict:
    """
...
async def list_all_releases() -> list:
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT album_artists, album_title, label, catalog_id, release_date FROM merged_data"
        )
...
async def get_new_catalog_id(release_type: ReleaseType) -> str:
    ...
    This is a placeholder implementation and should be replaced with your actual logic for generating catalog IDs.
    """
    with get_db_connection() as conn:
        cursor = conn.cursor()
    ...
    if max_id is None:
        ...
        return f"{release_type.value}{new_id_num:04d}"
